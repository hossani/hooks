...
export default function AddMovie({adding}) {
    ...
    let descRef = useRef();
        // État pour la note du film
    let [rate, setRate] = useState(0);
        // Fonction appelée lorsque la note est modifiée
    ...
    function submitted(ev){
        ev.preventDefault();
        // Création de l'objet représentant le film
        let movieObject = {title:titleRef.current.value, img:imgurlRef.current.value, description:descRef.current.value, posterURL:posurlRef.current.value, rating:rate};
               // Appel de la fonction de rappel pour ajouter le film
        adding(movieObject);
    }
